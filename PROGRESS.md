...
| P4 — Status hero rebuilt on the third reference | ✅ Done (2026-08-03) | Owner: *"I want to make this look more cooler"* + a reference pin. The pin turned out to be a **light-mode green/orange nutrition tracker** — its skin contradicts §5 outright, so only its structure was taken (DECISIONS D34). Today's hero is now: a **week strip** (Mon-start, three distinct states — cleared / missed / not yet), a **segmented ring** with the level numeral inside it and the XP-to-next caption below, two flanking counters (XP today, day streak), and three small meters (Cleared, Week, Best). The 20-cell straight rail became a 24-segment arc: bending it frees the widest element on the screen to carry the number the user opened the app to see. The whole block sits on a full-bleed **shelf** with a chamfered bottom edge, so the quest log scrolls against it. New engine function `weekStrip` with **7 tests (96 total)** — the week-boundary and month-boundary off-by-ones belong in a failing test, not in a screenshot someone squints at. The week strip deep-links into Calendar via `?day=` |

| P5 — Icons with mass, and a Profile with a record | ✅ Done (2026-08-03) | Owner: *"improve the profile page, add icons, use above reference for the kind of icons I want, reuse same in quest cards."* The reference's category marks are 3D-rendered illustrations — §5 forbids illustrated art and §3 forbids the asset budget, so the **treatment** was taken rather than the artwork (DECISIONS D36). All eight category glyphs redrawn **duotone** (filled body under a stroked outline) so they read as objects rather than hairlines, and Stock Trading became candlesticks — a rising arrow was already every other progress mark in the app. New `CategorySlot`: a notched inventory-slot tile carrying the skill's own colour as fill, edge and glow, shared by the quest row and the Profile skill list so one component defines what a category looks like. Profile's skill rows were a bar with an 8px diamond whose readout was the **level** — `1` for every skill until ~100 completions, so the ranking column said the same thing seven times; they now lead with the icon and report XP, with the level beside it. Profile also gained a **lifetime record strip** (cleared / active days / total XP / best streak) — it previously said nothing Today didn't already say. Dead `StatBar` deleted |
...
| P10 — Missed and overdue quests are marked | ✅ Done (2026-08-06) | Owner: *"missed quests should be marked with different color."* Two pure engine functions with **8 new tests (146 total)**: `questDayState` (done / skipped / missed / pending / upcoming for one quest on one day) and `isOverdue` (a todo past its due date and still open). Three surfaces: the **calendar day list** (red pip + `MISSED` tag, with `SKIPPED` kept visually distinct — §7 went out of its way to record that difference and the screen was throwing it away), the **month grid** (a third marker: red outline for a past day with an unlogged obligation, behind the blue "something was completed" dot, because "you did something" is the truer headline for a day), and **Today** (a red spine + `OVERDUE` tag on a todo past its due date — previously a quest due last week looked identical to one due tomorrow). Calendar now also loads skips, which it never did, so it could not previously tell a deliberate skip from a miss at all. **The rule the tests protect: today is never missed** — an open day is pending, which is what §2 and the streak engine both already say |

| P11 — Skipped gets its own colour; state stops overwriting identity | ✅ Done (2026-08-06) | Owner: *"assign a different color to skipped quests."* Teal `#2DD4BF`, chosen with the `dataviz` validator rather than by eye — the intuitive amber/orange scores **ΔE 11.3 normal / 9.8 deutan** against the missed red, the classic red-orange trap, while teal scores 35.8/17.5 (D46). Checking it also caught a flaw in P10: the missed state was **overwriting the difficulty pip**, putting danger red into the same 8px slot as the five-colour rarity ramp, where red↔Trivial grey measures CVD ΔE 7.5. Fixed by a rule that now holds everywhere — **identity colours (difficulty pip, category spine) are never overwritten; state lives on the tag and the panel edge**, and is always spelled out in words as well |
| P12 — iOS: the guidance was backwards, and there is now an escape hatch | ✅ Done (2026-08-06) | The owner's friend reported the iOS failure **worked in a private tab** — the opposite of what the failure screen led with. A private tab starts with an empty storage bucket, so "works in incognito, fails normally" is the signature of broken *local state*, not a permissions block. The causes are reordered, and a two-step **Reset local data** was added: it is the only route back for someone whose pool will not open. Two real bugs were found building it. `removeEntry` throws **NoModificationAllowedError** while the sahpool VFS holds its sync access handles, so the reset deleted nothing — and reported success, then reloaded, looking like a no-op. It now terminates the app's worker first (measured: handle held → `NoModificationAllowedError`; after `terminate()` + 150ms → `removed`) and **throws with an actionable message** if it is still blocked |
...
for a glow. A filter on a *child* of an `<svg>` gets its own filter region, and Chromium
rasterised that region as a visibly lighter **rectangle** behind every locked crest — thirty
grey boxes in a grid of hexagons. Only visible by zooming into a screenshot; every assertion
passed. It was also thirty filter passes. Removed, and the accent colour carries the emphasis
on its own (GOTCHAS: put filters on the `<svg>` root, not on a path inside it).
